...
if st.button(L["b_start"], type="primary"):
    if not cheie_finala:
        ...
        with st.spinner(L["coada_msg"] if este_blocat else L["spinner"]):
            with coada_globala:
                try:
                    client = genai.Client(api_key=cheie_finala); response = None
                    for incercare in range(3):
                        try:
                            response = client.models.generate_content(model="gemini-2.5-flash", contents=f"{L['prompt']}\n\n{contract_final_text}")
                            break
                    ...
                    if response:
                        if st.session_state["limba"] == "RO":
                            text_disclaimer_protectie = "⚠️ **NOTĂ IMPORTANTĂ DE SIGURANȚĂ JURIDICĂ:**\nAcest raport este generat în mod automat de un algoritm bazat pe inteligență artificială și are un scop pur educativ, informativ și de orientare comercială generală. Prezentele puncte reprezintă o opinie algoritmică strict orientativă și NU constituie consultanță juridică autorizată. Această platformă nu înlocuiește sub nicio formă un specialist în drept.\n\n**Pentru decizii contractuale oficiale, sfaturi de specialitate și o expertiză legală autorizată, vă rugăm și vă recomandăm insistent să consultați o casă de avocatură autorizată sau un cabinet de avocatură înscris în Barou.**\n\n---\n\n"
                        else:
                            text_disclaimer_protectie = "⚠️ **IMPORTANT LEGAL DISCLAIMER:**\nThis report is automatically generated by an AI algorithm and is intended strictly for educational, informational, and general business guidance purposes. These points represent a strictly indicative algorithmic opinion and DO NOT constitute authorized legal advice. This platform does not replace a legal professional.\n\n**For official contractual decisions, professional advice, and authorized legal expertise, please and strongly we advise you to consult an authorized law firm or a licensed attorney.**\n\n---\n\n"
                        st.session_state["rezultat_analiza"] = text_disclaimer_protectie + response.text; st.success(L["succes"]); st.rerun()
