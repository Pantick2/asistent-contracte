...
def run_analiza():
    ...
    # Interfața aplicației se randează doar după ce bifa este activată
    ...
    if api_cheie_utilizator.strip():
        ...
        st.sidebar.info(f"{L['side_d']} ({st.session_state['numar_utilizari']}/{LIMITA_UTILIZARI_GRATUITE}).")
    ...
    if text_manual.import streamlit as st
from google import genai
from google.genai import types
import docx
import pypdf
...
if foloseste_mod_demo: st.session_state["numar_utilizari"] += 1st.session_state["rezultat_analiza"] = response.textst.success(L["s_analiza"])st.rerun()except Exception as e:st.error(f"Eroare: {str(e)}")if "rezultat_analiza" in st.session_state:st.markdown(L["r_titlu"])st.markdown(st.session_state["rezultat_analiza"])st.download_button(label=L["b_down"], data=st.session_state["rezultat_analiza"], file_name="analiza.txt", mime="text/plain")st.markdown(f"{L['subsol']}", unsafe_allow_html=True)run_analiza()():
        contract_final_text = text_manual
